...
const Storage = {
        ...
        if (raw) {
            try {
                ...
                Object.keys(STATE).forEach(clave => {
                    if (Object.prototype.hasOwnProperty.call(migrado, clave)) STATE[clave] = migrado[clave];
                });
            } catch (err) {
                console.warn('Estado local corrupto; se restauran valores iniciales.', err);
                try {
                    localStorage.setItem(CONFIG.STORAGE_KEY + '_corrupto_' + Date.now(), raw);
                } catch (_) {}
    ...
    migrate(data) {
        ...
        const aliases = {
            ...
            'Extensión de tríceps por encima de la cabeza (viernes)': 'Extensión de tríceps por encima de la cabeza'
        };
        ...
        if (Array.isArray(d.records)) {
            d.records.forEach(r => { if (aliases[r.exerciseName]) r.exerciseName = aliases[r.exerciseName]; });
        }
        return d;
    },
    ...
    _normalizarEstado() {
        ...
        STATE.recordatorios.freqMediciones = Math.max(1, Number(STATE.recordatorios.freqMediciones) || 2);
        STATE.recordatorios.freqFotos = Math.max(1, Number(STATE.recordatorios.freqFotos) || 4);
        if (!STATE.recordatorios.ultimaMedicion) STATE.recordatorios.ultimaMedicion = CONFIG.FECHA_REFERENCIA_MEDICIONES;
        ...
        if (!STATE.progresion || typeof STATE.progresion !== 'object') STATE.progresion = {};
    ...
    async exportar() {
        try {
            ...
            if (android) {
                try {
                    if (typeof android.exportBackup === 'function') { android.exportBackup(contenido, nombre); UI.toast('✅ Backup enviado a Android', 'success'); return; }
                    if (typeof android.saveBackup === 'function') { android.saveBackup(contenido, nombre); UI.toast('✅ Backup enviado a Android', 'success'); return; }
                } catch (e) { console.warn('Puente Android no disponible:', e); }
            }
            ...
            if (navigator.share && navigator.canShare) {
                try {
                    ...
                    if (navigator.canShare({ files: [file] })) {
                        await navigator.share({ title: 'Backup de NicoGym', files: [file] });
                        UI.toast('✅ Backup compartido/guardado', 'success');
                        return;
                    }
                } catch (e) { if (e?.name !== 'AbortError') console.warn('Web Share no disponible:', e); }
            }
    ...
    importar(file) {
        if (!file) { UI.toast('❌ No se seleccionó ningún archivo', 'error'); return; }
        const reader = new FileReader();
        reader.onload = async e => {
            try {
                const datos = JSON.parse(e.target.result);
                if (!datos || typeof datos !== 'object' || Array.isArray(datos)) throw new Error('Formato de backup no válido');
                const esNuevo = datos.app === 'NicoGym' && datos.state && typeof datos.state === 'object';
                if (esNuevo) {
                    const v = Number(datos.version);
                    if (!Number.isInteger(v) || v < 1 || v > CONFIG.BACKUP_VERSION) throw new Error('Versión de backup no compatible');
                }
                const datosEstado = esNuevo ? datos.state : datos;
                const campos = {
                    mediciones:'array', historialEntrenos:'array', diasNoFumar:'array', diasEntrenados:'array',
                    checks:'object', recordatorios:'object', records:'array', evolution:'object', config:'object', ajustes:'object'
                };
                for (const [campo, tipo] of Object.entries(campos)) {
                    if (datosEstado[campo] === undefined) continue;
                    const ok = tipo === 'array' ? Array.isArray(datosEstado[campo]) : datosEstado[campo] && typeof datosEstado[campo] === 'object' && !Array.isArray(datosEstado[campo]);
                ...
                Object.keys(STATE).forEach(k => { if (Object.prototype.hasOwnProperty.call(estadoImportado, k)) STATE[k] = estadoImportado[k]; });
                this._normalizarEstado();
                CONFIG.TEMPORIZADOR_DESCANSO = STATE.config.temporizadorDescanso === true;
                ...
                UI.toast(`❌ Error al importar: ${err.message || 'formato inválido'}`, 'error');
            }
        };
